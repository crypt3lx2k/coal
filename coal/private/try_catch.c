...
static thread_local struct coal_private_stack exception_s =
  COAL_PRIVATE_STACK_INITIALIZER;

bool coal_private_try_isEmpty (void) {
  return coal_private_stack_isEmpty(&exception_s);
}

struct _coal_private_try_context *
coal_private_try_pop (void) {
  return coal_private_stack_pop(&exception_s);
}

void
coal_private_try_push (struct _coal_private_try_context * c) {
  return coal_private_stack_push(&exception_s, c);
}
